    ...
    if varlen_sparse_feature_columns and len(varlen_sparse_feature_columns) > 0:
        for feat in varlen_sparse_feature_columns:
            ...
            emb = Embedding(feat.vocabulary_size, feat.embedding_dim,
                            ...
                            mask_zero=seq_mask_zero)  # 变长特征序列需要加入mask zero忽略padding部分的0值
            emb.trainable = feat.trainable
            sparse_embedding[feat.embedding_name] = emb
        ...
        if (len(return_feat_list) == 0 or feature_name in return_feat_list):
            if fc.use_hash:
                lookup_idx = Hash(fc.vocabulary_size, mask_zero=(feature_name in mask_feat_list),
                    ...
                    sparse_input_dict[feature_name])  # Embedding input为特征原始input hash后的结果
            else:
                lookup_idx = sparse_input_dict[feature_name]  # Embedding input为特征原始input
    ...
    for fc in varlen_sparse_feature_columns:
        ...
        if feature_length_name is not None:
            ...
            vec = SequencePoolingLayer(combiner, supports_masking=False)(
                [
                    seq_input,  # 序列embed内容
                    features[feature_length_name]  # 序列长度用于生成mask，用于忽略序列的padding zero部分
                ])
        else:
            if fc.weight_name is not None:
