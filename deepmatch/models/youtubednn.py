...
def YoutubeDNN(user_feature_columns, item_feature_columns, num_sampled=5,
    ...
    # 构建稀疏特征到embedding的dict
    embedding_matrix_dict = create_embedding_matrix(user_feature_columns + item_feature_columns, l2_reg_embedding,
                                                    seed=seed)

    user_features = build_input_features(user_feature_columns)  # 构建用户特征的input
    ...
    # 使用flatten和concat链接用户所有的sparse特征和dense特征
    ...
    item_embedding_weight = NoMask()(item_embedding_matrix(item_index))  # 生成所有物品的embedding输出，将他作为sampled soft max的权重输入

    pooling_item_embedding_weight = PoolingLayer()([item_embedding_weight])  # 实际上没有进行pooling操作

    # 通过sampled soft max loss代替full soft max loss加速模型拟合
    output = SampledSoftmaxLayer(num_sampled=num_sampled)(
        [pooling_item_embedding_weight, user_dnn_out, item_features[item_feature_name]])
    model = Model(inputs=user_inputs_list + item_inputs_list, outputs=output)

    # 设置用户embedding模型输入和输出
    model.__setattr__("user_input", user_inputs_list)
    model.__setattr__("user_embedding", user_dnn_out)

    # 设置物品embedding模型的输入和输出
